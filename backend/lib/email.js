...
// attachments (optional): [{ filename, content: Buffer }] — content is
// converted to base64 here, matching what Resend's API expects
// (https://resend.com/docs/dashboard/emails/attachments — local-file
// attachments use { filename, content: <base64 string> }).
    ...
    const res = await fetch(RESEND_API_URL, {
      ...
      body: JSON.stringify({
        ...
        ...(attachments && attachments.length
          ? { attachments: attachments.map((a) => ({ filename: a.filename, content: Buffer.isBuffer(a.content) ? a.content.toString('base64') : a.content })) }
          : {}),
      }),
    });
    if (!res.ok) {
...
// invoicePdfBuffer (optional): pass a Buffer to attach the invoice PDF to
// the confirmation email — generated by backend/lib/invoice.js's
// generateInvoiceBuffer, which email.js can't do itself (it would need
// dataStore.js for the product/HSN lookups, and dataStore.js already
// requires email.js — a require cycle). See backend/routes/orders.js, the
// caller that builds the buffer and passes it in here.
async function sendOrderConfirmedEmail(order, invoicePdfBuffer) {
  ...
  const html = `
    ...
    <p>${invoicePdfBuffer ? 'Your invoice is attached as a PDF. ' : ''}We'll email you again once your order ships.</p>
    <p>— Groove Organics</p>
  `;
  return sendEmail({
    ...
    html,
    attachments: invoicePdfBuffer ? [{ filename: `invoice-${order.order_number}.pdf`, content: invoicePdfBuffer }] : undefined,
  });
}
